...
int fbopen() {
    ...
    if (ioctl(fd, FBIOGET_FSCREENINFO, &fb_finfo)) return FBOPEN_FSCREENINFO;
    if (ioctl(fd, FBIOGET_VSCREENINFO, &fb_vinfo)) return FBOPEN_VSCREENINFO;
    if (fb_vinfo.bits_per_pixel != 32) return FBOPEN_BPP;
...
void fbputchar(char c, int row, int col) {
    ...
    unsigned char *pixel, *left = framebuffer +
        (row * FONT_HEIGHT * fb_finfo.line_length) +
        (col * FONT_WIDTH * BITS_PER_PIXEL / 8);
    ...
    for (y = 0; y < FONT_HEIGHT; y++, left += fb_finfo.line_length) {
        ...
        for (x = 0; x < FONT_WIDTH; x++) {
            if (pixels & mask) {
                pixel[0] = 255; pixel[1] = 255; pixel[2] = 255; pixel[3] = 0; // White
            } else {
                pixel[0] = 0; pixel[1] = 0; pixel[2] = 0; pixel[3] = 0; // Black
            }
            pixel += 4;
            mask >>= 1;
        }
...
void scroll_text_up() {
    int start_row = 41;  // Row from which scrolling should start
    int start_offset = fb_finfo.line_length * start_row * FONT_HEIGHT;
    int scroll_height = (fb_vinfo.yres - start_row * FONT_HEIGHT) - FONT_HEIGHT;

    // Move only the part of the framebuffer above row 41
    memmove(framebuffer + start_offset, 
            framebuffer + start_offset + fb_finfo.line_length * FONT_HEIGHT,
            fb_finfo.line_length * scroll_height);

    // Clear the last row after scrolling
    memset(framebuffer + start_offset + fb_finfo.line_length * scroll_height, 0, 
